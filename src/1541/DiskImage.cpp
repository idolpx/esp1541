...
void DiskImage::DumpTrack(unsigned track)
{
	...
	Debug_printf("track = %d trackLength = %d\r\n", track, trackLength);
...
bool DiskImage::WriteD64(char* name)
{
	...
	if (res == FR_OK)
	{
		...
		Debug_printf("Writing D64 file...\r\n");

		memset(d64data, 0, sizeof(d64data));

		if (!GetID(34, id))
		{
			Debug_printf("Cannot find directory sector.\r\n");
			return false;
		}
		d64ptr = d64data;
		...
		{
			SetACTLed(false);
			Debug_printf("Cannot write d64 data.\r\n");
			f_close(&fp);
			return false;
		}
		...
		Debug_printf("Converted %d blocks into D64 file\r\n", blocks_to_save);

		return true;
	}
	else
	{
		Debug_printf("Failed to open %s for write\r\n", fileInfo->fname);
		return false;
	}
}
...
bool DiskImage::WriteD71()
{
	...
	//	{
	//		SetACTLed(false);
	//		Debug_printf("Cannot write d71 data.\r\n");
	//		f_close(&fp);
	//		return false;
	//	}
	...
	//	Debug_printf("Converted %d blocks into D71 file\r\n", blocks_to_save);
...
bool DiskImage::OpenG64(const FILINFO* fileInfo, unsigned char* diskImage, unsigned size)
{
	...
	if (memcmp(diskImage, "GCR-1541", 8) == 0)
	{
		...
		//Debug_printf("Is G64 %08x\r\n", hash);

		unsigned char numTracks = diskImage[9];
		//Debug_printf("numTracks = %d\r\n", numTracks);
		...
		for (track = 0; track < numTracks; ++track)
		{
			...
			else
			{
				...
				//Debug_printf("trackLength = %d offset = %d\r\n", trackLength, offset);
				trackData += 2;
...
static bool WriteDwords(FIL* fp, uint32_t* values, uint32_t amount)
{
	uint32_t index;
...
bool DiskImage::WriteG64(char* name)
{
	...
	if (res == FR_OK)
	{
		...
		Debug_printf("Writing G64 file...\r\n");
		//Debug_printf("G64 Track Length = %d", G64_TRACK_MAXLEN);
		...
		{
			SetACTLed(false);
			Debug_printf("Cannot write G64 header.\r\n");
			f_close(&fp);
			return false;
		}
		SetACTLed(false);
		...
		for (track = 0; track < MAX_HALFTRACKS_1541; track += track_inc)
		{
			...
			{
				SetACTLed(false);
				Debug_printf("Cannot write track data.\r\n");
				f_close(&fp);
				return false;
			}
			SetACTLed(false);
		...
		f_close(&fp);
		Debug_printf("nSuccessfully saved G64\r\n");
...
bool DiskImage::OpenNIB(const FILINFO* fileInfo, unsigned char* diskImage, unsigned size)
{
	...
	if (memcmp(diskImage, "MNIB-1541-RAW", 13) == 0)
	{
		...
		while (diskImage[0x10 + h_index])
		{
			...
			Debug_printf("Converting NIB track %d (%d.%d)\r\n", track, track >> 1, track & 1 ? 5 : 0);
		...
		Debug_printf("Successfully parsed NIB data for %d tracks\n", t_index);
		diskType = NIB;
		return true;
	}
	return false;
...
bool DiskImage::WriteNIB()
{
	...
	if (res == FR_OK)
	{
		...
		Debug_printf("Converting to NIB format...\n");
		...
		Debug_printf("nSuccessfully saved NIB\r\n");
...
bool DiskImage::WriteNBZ()
{
	...
	if (WriteNIB())
	{
		...
		if (res == FR_OK)
		{
			...
			f_close(&fp);
			Debug_printf("Reloaded %s - %d for compression\r\n", fileInfo->fname, bytesRead);
...
bool DiskImage::OpenT64(const FILINFO* fileInfo, unsigned char* diskImage, unsigned size)
{
	...
	{
		uint16_t version = diskImage[0x20] | (diskImage[0x21] << 8);
		uint16_t entries = diskImage[0x22] | (diskImage[0x23] << 8);
		uint16_t entriesUsed = diskImage[0x24] | (diskImage[0x25] << 8);
		char name[25] = { 0 };
		for (int i = 0; i < 24; ++i)
		{
		...
		unsigned char* newDiskImage = (unsigned char*)heap_caps_malloc(READBUFFER_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
		...
		if (newDiskImage)
		{
			...
			if (length)
			{
				...
				for (entryIndex = 0; entryIndex < entriesUsed; ++entryIndex)
				{
					...
					uint8_t type = diskImage[offset];
					uint8_t fileType = diskImage[offset + 1];
					if (fileType == DIRECTRY_ENTRY_FILE_TYPE_PRG)
					{
						uint16_t startAddress = diskImage[offset + 2] | (diskImage[offset + 3] << 8);
						uint16_t endAddress = diskImage[offset + 4] | (diskImage[offset + 5] << 8);
						unsigned length = endAddress - startAddress;

						uint32_t fileOffset = diskImage[offset + 8] | (diskImage[offset + 9] << 8) | (diskImage[offset + 10] << 16) | (diskImage[offset + 11] << 24);
						strncpy(nameEntry, (const char*)(diskImage + offset + 0x10), 16);

						unsigned char* data = (unsigned char*)heap_caps_malloc(length + 2, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
...
bool DiskImage::GetDecodedSector(uint32_t track, uint32_t sector, uint8_t* buffer)
{
	if (track > 0)
	{
		track = (track - 1) * 2;
...
unsigned DiskImage::CreateNewDiskInRAM(const char* filenameNew, const char* ID, unsigned char* destBuffer)
{
	...
	Debug_printf("CreateNewDiskInRAM %s\r\n", filenameNew);
...
unsigned char* DiskImage::RAMD64AddDirectoryEntry(unsigned char* ramD64, const char* name, const unsigned char* data, unsigned length)
{
	...
	for (directorySector = 1; directorySector < 19; ++directorySector)
	{
		...
		Debug_printf("directory sector offset = %d\r\n", sectorOffset);
		ptr = ramD64 + sectorOffset * 256;
...
int DiskImage::RAMD64FreeSectors(unsigned char* ramD64)
{
	...
	Debug_printf("sectorOffset bam = %d\r\n", sectorOffset);
