...
void M6502::ARR(void)
{
	...
	uint16_t carry = status & FLAG_CARRY;

	if (status & FLAG_DECIMAL)
	{
		uint16_t resultDEC = result;
		resultDEC |= carry << 8;
		resultDEC >>= 1;
		SetN(carry);
		SetZ(resultDEC == 0);
...
void M6502::SBC(void)
{
	uint16_t result = a - value - ((status & FLAG_CARRY) ? 0 : 1);
	if (status & FLAG_DECIMAL)
	{
		uint16_t tmp_a;
...
uint8_t M6502::BusRead(uint16_t address)
{
