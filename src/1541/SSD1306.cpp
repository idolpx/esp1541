...
SSD1306::SSD1306(int BSCMaster, uint8_t address, unsigned width, unsigned height, int flip, LCD_MODEL type)
	: BSCMaster(BSCMaster)
	, address(address)
	, type(type)
	, flip(flip)
	...
	, height(height)
{
	...
	frame = (unsigned char *)heap_caps_malloc(sizeof_frame, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
	oldFrame = (unsigned char *)heap_caps_malloc(sizeof_frame, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
...
void SSD1306::SendDataLong(void* data, uint8_t length)
{
	char buffer[15];
...
void SSD1306::SetDataPointer(uint8_t page, uint8_t col)
{
...
void SSD1306::RefreshTextRows(uint32_t start, uint32_t amountOfRows)
{
	unsigned int i;
...
void SSD1306::RefreshPage(uint32_t page)
{
	if (page >= height/8)
		return;
...
void SSD1306::SetVCOMDeselect(uint8_t value)
{
