...
extern void DisplayMessage(int x, int y, bool LCD, const char* message, uint32_t textColour, uint32_t backgroundColour);
...
static uint8_t DirectoryHeader[] =
{
...
static const uint8_t DirectoryBlocksFree[] = {
	1, 1,	// Next line pointer
...
static uint8_t* InsertNumber(uint8_t* msg, uint8_t value)
{
	if (value >= 100)
	{
		*msg++ = '0' + value / 100;
...
{
	uint8_t* ptr = DirectoryHeader + VERSION_OFFSET_IN_DIR_HEADER;
...
void Error(uint8_t errorCode, uint8_t track = 0, uint8_t sector = 0)
{
	char* msg = "UNKNOWN";
	switch (errorCode)
	{
...
bool IEC_Commands::WriteIECSerialPort(uint8_t data, bool eoi)
{
...
bool IEC_Commands::ReadIECSerialPort(uint8_t& byte)
{
	byte = 0;
...
IEC_Commands::UpdateAction IEC_Commands::SimulateIECUpdate(void)
{
	if (IEC_Bus::IsReset())
	{
		...
		do
		{
			//Debug_printf("Reset during SimulateIECUpdate\r\n");
			IEC_Bus::ReadBrowseMode();
...
void IEC_Commands::Memory(void)
{
	...
	if (channel.cursor > 2)
	{
		...
		if (code == 'R' || code == 'W' || code == 'E')
		{
			...
			address = (u16)((uint8_t)(ptr[1]) << 8) | (u16)ptr[0];
			if (length > 2)
			{
				bytes = ptr[2];
				if (bytes == 0)
...
{
	for (uint32_t i = 0; i < channel.cursor; ++i)
	{
		uint8_t finalbyte = eoi && (channel.bytesSent == (channel.fileSize - 1));
...
void IEC_Commands::LoadFile()
{
	...
	if (channel.filInfo.fname[0] != 0)
	{
		...
		uint32_t bytesRead;
		channel.fileSize = (uint32_t)channel.filInfo.fsize;
		...
		else if (toupper((char)ext[1]) == 'T' && ext[2] == '6' && ext[3] == '4')
		{
			...
			if (bytesRead > 0)
			{
				...
				{
					Debug_printf("T64\r\n");
					uint16_t version = channel.buffer[0x20] | (channel.buffer[0x21] << 8);
					uint16_t entries = channel.buffer[0x22] | (channel.buffer[0x23] << 8);
					uint16_t entriesUsed = channel.buffer[0x24] | (channel.buffer[0x25] << 8);
					char name[25] = { 0 };
					strncpy(name, (const char*)(channel.buffer + 0x28), 24);

					Debug_printf("%x %d %d %s\r\n", version, entries, entriesUsed, name);
					...
					for (entryIndex = 0; entryIndex < entriesUsed; ++entryIndex)
					{
						...
						uint8_t type = channel.buffer[offset];
						uint8_t fileType = channel.buffer[offset + 1];
						uint16_t startAddress = channel.buffer[offset + 2] | (channel.buffer[offset + 3] << 8);
						uint16_t endAddress = channel.buffer[offset + 4] | (channel.buffer[offset + 5] << 8);
						uint32_t fileOffset = channel.buffer[offset + 8] | (channel.buffer[offset + 9] << 8) | (channel.buffer[offset + 10] << 16) | (channel.buffer[offset + 11] << 24);
						strncpy(nameEntry, (const char*)(channel.buffer + offset + 0x10), 16);

						Debug_printf("%d %02x %04x %04x %0x8 %s\r\n", type, fileType, startAddress, endAddress, fileOffset, nameEntry);
	...
	{
		//Debug_printf("Can't find %s", channel.buffer);
		Error(ERROR_62_FILE_NOT_FOUND);
	}
}
...
uint8_t IEC_Commands::GetFilenameCharacter(uint8_t value)
{
...
void IEC_Commands::AddDirectoryEntry(Channel& channel, const char* name, uint16_t blocks, int fileType)
{
	uint8_t* data = channel.buffer + channel.cursor;
	const uint32_t dirEntryLength = DIRECTORY_ENTRY_SIZE;
	int i = 0;
	int index = 0;
	bool diskImage = DiskImage::IsDiskImageExtention(name);

	//Debug_printf("name = %s blocks = %d", name, blocks);
...
void IEC_Commands::OpenFile()
{
	...
	//	P - program
	//	R - relative file
	uint8_t secondary = secondaryAddress;
...
void IEC_Commands::CloseFile(uint8_t secondary)
{
	Channel& channel = channels[secondary];
