...
extern void GlobalSetDeviceID(uint8_t id);
...
void FileBrowser::BrowsableListView::RefreshLine(uint32_t entryIndex, uint32_t x, uint32_t y, bool selected)
{
	char buffer1[128] = { 0 };
	char buffer2[256] = { 0 };
	uint32_t colour;
	RGBA BkColour = RGBA(0, 0, 0, 0xFF); //palette[VIC2_COLOUR_INDEX_BLUE];
	uint32_t columnsMax = columns;
...
void FileBrowser::BrowsableListView::Refresh()
{
	uint32_t index;
	uint32_t entryIndex;
	uint32_t x = positionX;
	uint32_t y = positionY;
...
{
	bool dirty = false;
	uint32_t numberOfEntriesMinus1 = list->entries.size() - 1;
	...
	if ((lcdPgUpDown && inputMappings->BrowsePageUpLCD()) || (!lcdPgUpDown && inputMappings->BrowsePageUp()))
	{
		if (list->currentIndex == offset)
		{
			...
			else offset = (uint32_t)offsetInWindow;
...
{
	uint32_t numberOfEntriesMinus1 = entries.size() - 1;

	bool dirty = false;
	uint32_t index;

	// Calculate the number of micro seconds since we were last called.
	uint32_t updateTime = read32(ARM_SYSTIMER_CLO);
	uint32_t timeDelta;
...
FileBrowser::FileBrowser(InputMappings* inputMappings, DiskCaddy* diskCaddy, ROMs* roms, uint8_t* deviceID, bool displayPNGIcons, ScreenBase* screenMain, ScreenBase* screenLCD, float scrollHighlightRate)
	...
	, displayingDevices(false)
{
...
#if not defined(EXPERIMENTALZERO)
	uint32_t columns = screenMain->ScaleX(80);
	uint32_t rows = (int)(38.0f * screenMain->GetScaleY());
	uint32_t positionX = 0;
	uint32_t positionY = 17;
	...
	if (screenLCD)
	{
		uint32_t columns = screenLCD->Width() / 8;
		uint32_t rows = screenLCD->Height() / screenLCD->GetFontHeight();
		uint32_t positionX = 0;
...
uint32_t FileBrowser::Colour(int index)
{
...
void FileBrowser::RefeshDisplayForBrowsableList(FileBrowser::BrowsableList* browsableList, int xOffset, bool showSelected)
{
	...
	uint32_t maxCharacters = screenMain->ScaleX(80);
...
{
#if not defined(EXPERIMENTALZERO)
	uint32_t textColour = Colour(VIC2_COLOUR_INDEX_LGREEN);
	uint32_t bgColour = Colour(VIC2_COLOUR_INDEX_GREY);
	char buffer[1024];
	...
	//uint32_t offsetX = screenMain->ScaleX(1024 - 320);
	...
	{
		uint32_t y = screenMain->ScaleY(STATUS_BAR_POSITION_Y);
...
void FileBrowser::DisplayPNG(FILINFO& filIcon, int x, int y)
{
	if (filIcon.fname[0] != 0)
	{
		...
		if (res == FR_OK)
		{
			char* PNG = (char*)heap_caps_malloc(filIcon.fsize, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
			if (PNG)
			{
				uint32_t bytesRead;
				SetACTLed(true);
				...
				{
					//Debug_printf("Opened PNG %s w = %d h = %d cif = %d\r\n", fileName, w, h, channels_in_file);
					screenMain->PlotImage((u32*)image, x, y, w, h);
				}
				else
				{
					//Debug_printf("Invalid PNG size %d x %d\r\n", w, h);
				}
#endif
				free(PNG);
			}
	...
	{
		//Debug_printf("Cannot find PNG %s\r\n", fileName);
	}
}
...
void FileBrowser::DisplayPNG()
{
	...
	if (displayPNGIcons && folder.current)
	{
		...
		uint32_t x = screenMain->ScaleX(1024) - PNG_WIDTH;
		uint32_t y = screenMain->ScaleY(616) - PNG_HEIGHT;
...
void FileBrowser::PopFolder()
{
	...
	if (f_getcwd(buffer, 1024) == FR_OK)
	{
		...
		else
		{
			...
			if (last_ptr)
			{
				uint32_t numberOfEntriesMinus1 = folder.entries.size() - 1;
...
bool FileBrowser::SelectROMOrDevice(uint32_t index)
...
{
#if not defined(EXPERIMENTALZERO)
	uint32_t bgColour = palette[VIC2_COLOUR_INDEX_BLUE];
...
{
	char buffer[256];
	uint32_t textColour = RGBA(0, 0, 0, 0xff);
	uint32_t bgColour = RGBA(0xff, 0xff, 0xff, 0xff);
	uint32_t x = 0; // 43 * 8
	uint32_t y;
...
void FileBrowser::DisplayDiskInfo(DiskImage* diskImage, const char* filenameForIcon)
{
	...
	int charIndex;
	uint32_t fontHeight = screenMain->GetFontHeightDirectoryDisplay();
	uint32_t x = 0;
	uint32_t y = 0;
	char bufferOut[128] = { 0 };
	uint32_t textColour = palette[VIC2_COLOUR_INDEX_LBLUE];
	uint32_t bgColour = palette[VIC2_COLOUR_INDEX_BLUE];

	uint32_t usedColour = palette[VIC2_COLOUR_INDEX_RED];
	uint32_t freeColour = palette[VIC2_COLOUR_INDEX_LGREEN];
	uint32_t thisColour = 0;
	uint32_t BAMOffsetX = screenMain->ScaleX(400);

	uint32_t bmBAMOffsetX = screenMain->ScaleX(1024) - PNG_WIDTH;
	uint32_t x_px = 0;
	uint32_t y_px = 0;
...
//screenMain->PrintText(false, x_px, PNG_HEIGHT+30, bufferOut, textColour, bgColour);
		...
		for (bamTrack = 0; bamTrack <= lastTrackUsed; ++bamTrack)
		{
			...
			{
				uint32_t bits = buffer[bamOffset + 1 + (bit >> 3) + bamTrack * BAM_ENTRY_SIZE];
			...
			while (!complete)
			{
				...
				if (diskImage->GetDecodedSector(track, sectorNo, buffer))
				{
					...
					for (entry = 0; entry < 8; ++entry)
					{
						...
						if (!done)
						{
							uint8_t fileType = buffer[DIR_ENTRY_OFFSET_TYPE + entryOffset];
							uint16_t blocks = (buffer[DIR_ENTRY_OFFSET_BLOCKS + entryOffset + 1] << 8) | buffer[DIR_ENTRY_OFFSET_BLOCKS + entryOffset];
