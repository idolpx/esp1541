...
static const uint8_t snoopBackCommand[] = {
...
extern "C"
{
	...
	void usDelay(unsigned nMicroSeconds)
	{
		unsigned before;
		unsigned after;
		for (uint32_t count = 0; count < nMicroSeconds; ++count)
		{
...
extern uint8_t read6502ExtraRAM(uint16_t address);
extern void write6502(uint16_t address, const uint8_t value);
extern void write6502ExtraRAM(uint16_t address, const uint8_t value);
extern uint8_t read6502_1581(uint16_t address);
extern void write6502_1581(uint16_t address, const uint8_t value);
...
//void UpdateUartControls(bool refreshStatusDisplay, bool LED, bool Motor, bool ATN, bool DATA, bool CLOCK, uint32_t Track, uint32_t romIndex)
//{
...
void UpdateScreen()
{
	...
	uint32_t oldTrack = 0;
	uint32_t textColour = COLOUR_BLACK;
	uint32_t bgColour = COLOUR_WHITE;
	uint32_t oldTemp = 0;
  ...
  //bool refreshUartStatusDisplay;
	...
	{
		bool value;
		uint32_t y = screen.ScaleY(STATUS_BAR_POSITION_Y);
...
static void PlaySoundDMA()
{
	...
	write32(DMA0_BASE + DMA_CONBLK_AD, (uint32_t)&dmaSoundCB);
	write32(DMA0_BASE + DMA_CS, DMA_ACTIVE);
}
#endif

void GlobalSetDeviceID(uint8_t id)
{
	deviceID = id;
...
EXIT_TYPE Emulate1541(FileBrowser* fileBrowser)
{
	...
	uint32_t hash = pi1541.drive.GetDiskImage()->GetHash();
...
extern "C" 
{
	void run_core() 
	{
		...
		Debug_printf("emulator running on core %d\r\n", _get_core());
		emulator();
	}
}
...
static void CheckOptions()
{
	...
	uint32_t widthText, heightText;
	uint32_t widthScreen = screen.Width();
	uint32_t heightScreen = screen.Height();
	uint32_t xpos, ypos;

	deviceID = (uint8_t)options.GetDeviceID();
	...
	if (FontROMName)
	{
			...
			|| (FR_OK == f_open(&fp, FontROMName2, FA_READ)) )
		{
			...
			//Debug_printf("Read ROM %s from options\r\n", ROMName);
		}
	}
#endif
...
void UpdateFirmwareToSD()
{
	...
	if (SwitchDrive("USB01:"))
	{
		...
		if (f_getcwd(cwd, 1024) == FR_OK)
		{
			...
			if (found)
			{
				char* mem = (char*)heap_caps_malloc((uint32_t)filInfo.fsize, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
				if (mem)
				{
					FIL fp;
					uint32_t bytes;
					...
					if (res == FR_OK)
					{
						...
						res = f_read(&fp, mem, (uint32_t)filInfo.fsize, &bytes);
						f_close(&fp);
						if ((res == FR_OK) && ((uint32_t)filInfo.fsize == bytes))
						{
							if (SwitchDrive("SD:"))
							{
								if (f_chdir("\\") == FR_OK)
								{
									...
									if (FR_OK == f_open(&fp, firmwareName, FA_READ))
									{
										...
										} while (same && (bufferIndex < (uint32_t)filInfo.fsize));
										f_close(&fp);
									}
									...
									if (!same && (FR_OK == f_open(&fp, firmwareName, FA_CREATE_ALWAYS | FA_WRITE)))
									{
										...
										res = f_write(&fp, mem, (uint32_t)filInfo.fsize, &bytes);
										f_close(&fp);
									}
								}
							}
...
void DisplayMessage(int x, int y, bool LCD, const char* message, uint32_t textColour, uint32_t backgroundColour)
{
#if not defined(EXPERIMENTALZERO)
	char buffer[256] = { 0 };
...
extern "C"
{
	void kernel_main(unsigned int r0, unsigned int r1, unsigned int atags)
	{
		...
		Debug_printf("%d USB Mass Storage Devices found\r\n", numberOfUSBMassStorageDevices);

		USBKeyboardDetected = USPiKeyboardAvailable();
		if (!USBKeyboardDetected)
			Debug_printf("Keyboard not found\r\n");
		else
			Debug_printf("Keyboard found\r\n");

		//if (!USPiMouseAvailable())
		//	Debug_printf("Mouse not found\r\n");
		//else
		//	Debug_printf("Mouse found\r\n");
		...
		if (!options.SoundOnGPIO())
		{
			dmaSound = (u32*)heap_caps_malloc(Sample_bin_size * 4, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
