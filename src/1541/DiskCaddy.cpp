...
static const uint32_t screenPosXCaddySelections = 240;
static const uint32_t screenPosYCaddySelections = 280;
static char buffer[256] = { 0 };
static uint32_t white = RGBA(0xff, 0xff, 0xff, 0xff);
static uint32_t red = RGBA(0xff, 0, 0, 0xff);
static uint32_t redDark = RGBA(0x88, 0, 0, 0xff);
static uint32_t redMid = RGBA(0xcc, 0, 0, 0xff);
static uint32_t grey = RGBA(0x88, 0x88, 0x88, 0xff);
static uint32_t greyDark = RGBA(0x44, 0x44, 0x44, 0xff);
...
bool DiskCaddy::Insert(const FILINFO* fileInfo, bool readOnly)
{
	...
	if (res == FR_OK)
	{
		...
		if (success)
		{
			Debug_printf("Mounted into caddy %s - %d\r\n", fileInfo->fname, bytesRead);
		}
	}
	else
	{
...
void DiskCaddy::ShowSelectedImage(uint32_t index)
{
...
bool DiskCaddy::Update()
{
	uint32_t y;
	uint32_t x;
	uint32_t caddyIndex = GetSelectedIndex();
